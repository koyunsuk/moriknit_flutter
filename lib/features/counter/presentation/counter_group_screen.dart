...
/// 카운터 라이브러리 — 상세(그룹) 화면.
///
/// 진입: 요약화면(CounterListScreen)의 카드 탭.
/// - [projectId] 가 비어있지 않으면 그 프로젝트에 연결된 카운터만.
/// - [projectId] 가 비어있으면 프로젝트 미연결(자유) 카운터만.
class CounterGroupScreen extends ConsumerWidget {
  final String projectId;
  final String title;

  const CounterGroupScreen({
    super.key,
    required this.projectId,
    required this.title,
  });

  bool get _isFree => projectId.isEmpty;
      ...
      body: SafeArea(
        ...
        data: (all) {
          ...
          final filtered = all.where((c) {
            if (_isFree) return c.projectId.isEmpty;
            return c.projectId == projectId;
          }).toList();

          bool isCounterDone(CounterModel c) {
            if (c.hasTargets && c.stitchProgress >= 1.0 && c.rowProgress >= 1.0) {
              return true;
            }
          ...
          final done = filtered.where(isCounterDone).length;
          final inProgress = filtered
              .where((c) =>
                  !isCounterDone(c) &&
                  (c.hasTargets || c.projectId.isNotEmpty))
              .length;
            ...
            children: [
              ...
              Positioned.fill(
                child: _SortableCounterList(
                  ...
                  onAdd: () => _showCounterStartSheet(
                    context, ref, isKorean, projects,
                    presetProjectId: _isFree ? '' : projectId,
                  ),
                  onTap: (id) => context.push('/counter/$id'),
                  emptyMessage: _isFree
                      ? (isKorean
                          ? '자유 카운터가 없어요.'
                          : 'No free counters yet.')
                      : (isKorean
                          ? '이 프로젝트의 카운터가 없어요.'
                          : 'No counters for this project yet.'),
                ),
              ),
            ],
          );
  ...
  Future<void> _showCreateCounterDialog(
    ...
    String? selectedProjectId = presetProjectId.isEmpty ? null : presetProjectId;
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText:
                        isKorean ? '목표 단수 (선택)' : 'Target rows (optional)',
                    hintText: '0',
                ...
                if (projects.isNotEmpty) ...[
                  ...
                  Container(
                    decoration: BoxDecoration(
                      border:
                          Border.all(color: C.mu.withValues(alpha: 0.3)),
                    ...
                    child: DropdownButton<String?>(
                      ...
                      hint: Text(
                          isKorean ? '연결 안 함' : 'No project',
                          style: T.body.copyWith(color: C.mu)),
                      items: [
                        DropdownMenuItem<String?>(
                          value: null,
                          child: Text(isKorean ? '연결 안 함' : 'No project',
                              style: T.body),
                        ),
                        ...projects.map(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final targetStitch =
                    int.tryParse(targetStitchCtrl.text.trim()) ?? 0;
                final targetRow =
                    int.tryParse(targetRowCtrl.text.trim()) ?? 0;
                final counter =
                    CounterModel.empty(uid: authUser.uid, name: name).copyWith(
        ...
        builder: (_, scrollCtrl) => Column(
          children: [
            ...
            Padding(
              ...
              child: Row(children: [
                Text(isKorean ? '카운터 추가' : 'Add counter', style: T.h3)
              ]),
            ),
            const SizedBox(height: 12),
            Expanded(
                      ...
                      children: [
                        Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(14)),
                            child: Icon(Icons.copy_rounded, color: C.lmD)),
                        const SizedBox(width: 14),
                        Expanded(
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                              ...
                              Text(
                                  isKorean
                                      ? '기존 카운터 복사로 시작'
                                      : 'Copy existing counter',
                                  style: T.bodyBold),
                              const SizedBox(height: 4),
                              Text(
                                  isKorean
                                      ? '기존 카운터를 복사해서 시작해요'
                                      : 'Duplicate an existing counter',
                                  style: T.caption.copyWith(color: C.mu)),
                            ])),
                        Icon(Icons.chevron_right_rounded, color: C.mu),
                      ],
  ...
  void _showCopyCounterSheet(
      BuildContext context, WidgetRef ref, bool isKorean) {
                      ...
                      onTap: () async {
                        ...
                        try {
                          await runWithMoriLoadingDialog<void>(
                            context,
                            message:
                                isKorean ? '복사하는 중입니다.' : 'Duplicating...',
                            subtitle: isKorean
                                ? '잠시만 기다려 주세요.'
                        ...
                        children: [
                          ...
                          Expanded(
                              child: Column(
                                ...
                                Text(
                                    isKorean
                                        ? '코 ${c.stitchCount} · 단 ${c.rowCount}'
                                        : 'Sts ${c.stitchCount} · Rows ${c.rowCount}',
                                    style: T.caption.copyWith(color: C.mu)),
                              ])),
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            name,
            style:
                T.caption.copyWith(color: C.lmD, fontWeight: FontWeight.w600),
          ),
        ],
      ),
    );
